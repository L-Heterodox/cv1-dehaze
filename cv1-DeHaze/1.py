...
def min_filter(img,r):
    ...
    return cv2.erode(img,kernel)#最小值滤波器，腐蚀
def get_A(img_haze,dark_channel,bins_l):
    hist,bins = np.histogram(dark_channel,bins=bins_l)#直方图
...
if __name__ == '__main__':
    I = cv2.imread('./photo/1.jpg')/255.0
    dark_channel = get_min_channel(I)
    dark_channel_1 = min_filter(dark_channel,r=7)
    cv2.imwrite("./dark_channel1.jpg", dark_channel_1*255)
    ...
    cv2.imwrite("./photo_after/1_3.jpg",J)

    #评估
    PSNR = PSNR(J,I*255)
    print(f"PSNR:{PSNR}")
    ssim = sk_cpt_ssim(J,I*255, win_size=11, data_range=255, multichannel=True,channel_axis=2)
    print(f"ssim:{ssim}")
